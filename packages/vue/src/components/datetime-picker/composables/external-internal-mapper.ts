...
import type { ModelTypeConverted, ModelValue, MonthModel, TimeModel, VueEmit } from '../interfaces'
...
export function useExternalInternalMapper(emit: VueEmit, props: AllPropsType, isInputFocused: Ref<boolean>) {
  ...
  const mapDateExternalToInternal = (value: Date | Date[]) => {
    if (props.modelAuto) {
      if (Array.isArray(value))
        return [parseModelType(value[0]), parseModelType(value[1])]
  ...
  const mapInternalDatesToExternal = (noSanitize: boolean = false) => {
    ...
    if (props.modelAuto)
      return getModelAutoForExternal()
    if (defaultedMultiDates.value.enabled)
      return getMultiDatesForExternal()
    if (Array.isArray(internalModelValue.value)) {
  ...
  const mapExternalToInternal = (value: ModelValue) => {
    if (!value || (Array.isArray(value) && !value.length))
      return null
    if (props.timePicker)
      return mapTimeExternalToInternal(convertType(value))
    if (props.monthPicker)
      return mapMonthExternalToInternal(convertType(value))
    if (props.yearPicker)
      return mapYearExternalToInternal(convertType(value))
    if (defaultedMultiDates.value.enabled)
      return mapMultiDateExternalToInternal(convertType(value))
    if (props.weekPicker)
      return mapWeekExternalToInternal(convertType(value))
  ...
  const emitModelValue = (returnOnly: boolean = false) => {
    ...
    if (props.monthPicker)
      return modeEmitter(getMonthVal, returnOnly)
    if (props.timePicker)
      return modeEmitter(getTimeVal, returnOnly)
    if (props.yearPicker)
      return modeEmitter(getYear, returnOnly)
    if (props.weekPicker)
      return emitWeekPicker(returnOnly)
  ...
  const checkBeforeEmit = (): boolean => {
    if (internalModelValue.value) {
      if (defaultedRange.value.enabled) {
        if (defaultedRange.value.partialRange)
