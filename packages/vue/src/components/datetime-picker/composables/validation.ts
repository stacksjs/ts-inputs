...
export function useValidation(props: PickerBasePropsType | AllPropsType) {
  const { defaultedFilters, defaultedRange, propDates, defaultedMultiDates } = useDefaults(props)

  const isDateDisabled = (date: Date) => {
    if (!propDates.value.disabledDates)
      return false
    if (typeof propDates.value.disabledDates === 'function')
      return propDates.value.disabledDates(getDate(date))
  ...
  const validateAboveMax = (date: Date) => {
    if (!propDates.value.maxDate)
      return false
    if (props.yearPicker)
      return getYear(date) > getYear(propDates.value.maxDate)
  ...
  const validateBellowMin = (date: Date) => {
    if (!propDates.value.minDate)
      return false
    if (props.yearPicker)
      return getYear(date) < getYear(propDates.value.minDate)
  ...
  const checkMinMaxRange = (secondDate: Date, modelValue: InternalModuleValue, index = 0): boolean => {
    if (shouldCheckMinMaxRange(modelValue, index) && isValidYear(secondDate)) {
      ...
      if (defaultedRange.value.minRange)
        return diff >= +defaultedRange.value.minRange
      if (defaultedRange.value.maxRange)
        return diff <= +defaultedRange.value.maxRange
    }
    return true
  }
  ...
  const isTimeValid = computed(() => (internalModelValue: InternalModuleValue): boolean => {
    if (!props.enableTimePicker || props.ignoreTimeValidation)
      return true
