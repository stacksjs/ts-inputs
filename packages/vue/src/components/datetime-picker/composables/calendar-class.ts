...
export function useCalendarClass(modelValue: WritableComputedRef<InternalModuleValue>, props: PickerBasePropsType) {
  const {
  ...
  const highlightedWeekDay = (day: ICalendarDay) => {
    if (typeof defaultedHighlight.value === 'function')
      return defaultedHighlight.value(day.value)
  ...
  const getModeClasses = (day: ICalendarDay) => {
    if (defaultedRange.value.enabled) {
      ...
      if (props.weekPicker)
        return weekPickerRangeClasses(day)
