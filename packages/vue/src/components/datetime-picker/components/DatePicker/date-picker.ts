...
import type { ICalendarDate, ICalendarDay, IMarker, TimeType, VueEmit, WeekStartNum } from '../../interfaces'
...
export function useDatePicker(props: PickerBasePropsType, emit: VueEmit, triggerCalendarTransition: (inst?: number) => void, updateFlow: () => void) {
  ...
  const shouldUpdateMonthView = (isAction: boolean) => {
    if (!defaultedConfig.value.keepViewOnOffsetClick || isAction)
      return true
  ...
  const getRangeFocusIndex = (dates: Date[]) => {
    if (defaultedMultiCalendars.value.count) {
      if (defaultedMultiCalendars.value.solo)
        return 0
  ...
  const handleRangeDatesSelect = (day: ICalendarDay, isNext: boolean) => {
    presetTempRange()
    if (defaultedRange.value.autoRange)
      return handleAutoRange(day, isNext)
    if (defaultedRange.value.fixedStart || defaultedRange.value.fixedEnd)
      return setFixedDateRange(day)
  ...
  const selectDate = (day: ICalendarDay, isNext = false): void => {
    if (isDisabled(day.value) || (!day.current && props.hideOffsetDates))
      return emit('invalid-date', day.value)
    clickedDate.value = JSON.parse(JSON.stringify(day))

    if (!defaultedRange.value.enabled)
      return handleSingleDateSelect(day)
